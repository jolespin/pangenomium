...
def main(args=None):
    ...
    parser_labels.add_argument("-z", "--cluster_prefix_zfill", type=int, default=0, help="Cluster prefix zfill.  Use 0 to add no zfill. Only applicable when --cluster_label_mode numeric. [Default: 0]") 
    ...
    parser_fasta.add_argument("-w", "--wrap", type=int, default=0, help = "Line wrap for fasta.  Use 0 for no wrapping. [Default: 0]")
    ...
    if opts.fasta:
        for id_cluster, nodes in tqdm(cluster_to_nodes.items(), "Writing fasta files for each cluster: {}".format(opts.output_fasta_directory)):
            with open_file_writer(os.path.join(opts.output_fasta_directory, "{}.{}".format(id_cluster, opts.output_fasta_extension))) as f:
                for id_node in sorted(nodes):
                    fasta_writer(
                        header=f"{id_node} {id_cluster}", 
                        seq=id_to_sequence[id_node], 
                        file=f, 
                        wrap=opts.wrap,
                    )
